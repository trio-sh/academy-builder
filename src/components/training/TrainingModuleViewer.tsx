 ...
 * { margin: 0; padding: 0; box-sizing: border-box; }
 ...
 if (!module) {
 return (
 <div className="fixed inset-0 z-[9999] bg-foreground/[0.05] flex items-center justify-center">
 <div className="text-center">
 <div className="w-16 h-16 border-4 border-foreground/25 border-t-transparent rounded-full animate-spin mx-auto mb-4" />
 ...
 return (
 <div className="fixed inset-0 z-[9999] bg-foreground/[0.05] text-white flex items-center justify-center">
 <div className="text-center max-w-md px-6">
 <div className="relative mb-8">
 <div className="w-24 h-24 rounded-full bg-foreground/[0.05] flex items-center justify-center mx-auto">
 <Sparkles className="w-12 h-12 text-foreground animate-pulse" />
 </div>
 <div className="absolute inset-0 w-24 h-24 mx-auto rounded-full border-2 border-foreground/30 border-t-indigo-500 animate-spin" />
 </div>
 ...
 <div className="w-full bg-foreground/[0.05] rounded-full h-2 mb-2">
 <div
 className="bg-foreground/[0.05] h-2 rounded-full transition-all duration-500"
 ...
 className="text-foreground hover:text-foreground hover:bg-foreground/30"
 title="Stop speaking"
 >
 ...
 size="sm"
 onClick={toggleMute}
 className={`${isMuted ? 'text-gray-500' : 'text-foreground'} hover:bg-black`}
 ...
 <span className="w-1 h-3 bg-foreground/10 rounded-full animate-pulse" />
 <span className="w-1 h-4 bg-foreground/10 rounded-full animate-pulse" style={{ animationDelay: '0.1s' }} />
 <span className="w-1 h-2 bg-foreground/10 rounded-full animate-pulse" style={{ animationDelay: '0.2s' }} />
 </div>
 )}
 </div>
 ...
 <Clock className={`w-4 h-4 ${isExtraTime ? 'ink-vermilion' : timeRemaining < 60 ? 'ink-vermilion' : 'text-gray-400'}`} />
 <span className={`font-mono font-semibold ${isExtraTime ? 'ink-vermilion' : timeRemaining < 60 ? 'ink-vermilion' : 'text-white'}`}>
 {formatTime(timeRemaining)}
 </span>
 {isExtraTime && <span className="text-xs ink-vermilion">+3</span>}
 </div>
 </div>
 ...
 <Clock className={`w-3 h-3 ${isExtraTime ? 'ink-vermilion' : timeRemaining < 60 ? 'ink-vermilion' : 'text-gray-400'}`} />
 <span className={`font-mono font-semibold text-xs ${isExtraTime ? 'ink-vermilion' : timeRemaining < 60 ? 'ink-vermilion' : 'text-white'}`}>
 {formatTime(timeRemaining)}
 </span>
 </div>
 </div>
 ...
 <motion.div
 className={`h-full bg-foreground/[0.05] ${module.color}`}
 initial={{ width: 0 }}
 ...
 <div className="flex flex-col gap-3 p-3 rounded-2xl bg-foreground/50 backdrop-blur border border-white/5">
 ...
 isCurrent
 ? `bg-foreground/[0.05] ${module.color} scale-125 shadow-lg shadow-indigo-500/30`
 : isCompleted
 ? 'bg-vermilion/10'
 : isUnlocked
 ? 'bg-foreground/10 hover:bg-foreground/10 hover:scale-110'
 : 'bg-foreground/50'
 }`}
 title={scene.title}
 >
 ...
 <span className="absolute left-8 top-1/2 -translate-y-1/2 text-xs text-gray-300 opacity-0 group-hover:opacity-100 transition-all duration-200 whitespace-nowrap bg-foreground/95 px-3 py-1.5 rounded-lg border border-white/30 shadow-xl">
 {index + 1}. {scene.title}
 </span>
 </button>
 );
 ...
 key={currentScene?.id}
 ref={contentRef}
 className="bg-foreground/[0.05] rounded-3xl border border-white/30 overflow-hidden backdrop-blur-sm shadow-2xl shadow-black/20"
 >
 {/* Scene header */}
 ...
 currentScene?.type === 'choice' ? 'bg-foreground/30' :
 ...
 {currentScene?.type === 'choice' && <Target className="w-5 h-5 text-foreground" />}
 {currentScene?.type === 'reflection' && <MessageSquare className="w-5 h-5 ink-vermilion" />}
 {currentScene?.type === 'quiz' && <FileText className="w-5 h-5 ink-vermilion" />}
 ...
 <span className="inline-block w-4 h-px bg-foreground/10" />
 ...
 <div className="w-12 h-12 rounded-full bg-foreground/[0.05] flex items-center justify-center text-white font-semibold text-lg shadow-lg shadow-indigo-500/20">
 ...
 return <div key={i} className="flex items-start gap-2 ml-2 my-1.5 text-gray-400"><span className="text-foreground mt-1">•</span><span dangerouslySetInnerHTML={{ __html: parsed }} /></div>;
 }
 if (line.match(/^\d+\./)) {
 ...
 <div className="mb-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-foreground/30 border border-foreground/20 text-sm text-foreground">
 ...
 ? showFeedback
 ? choice.isCorrect
 ? 'border-foreground/25 bg-vermilion/10'
 ...
 <div className="flex items-start gap-3">
 {currentSceneWithAI.choices?.find(c => c.id === selectedChoice)?.isCorrect ? (
 <CheckCircle className="w-5 h-5 ink-vermilion flex-shrink-0 mt-0.5" />
 ) : (
 ...
 className={`mt-6 bg-foreground/[0.05] ${module.color} hover:opacity-90`}
 >
 Submit Answer
 ...
 className="w-full h-40 p-4 rounded-xl bg-black border border-white/30 text-white placeholder-gray-500 focus:border-foreground/50 focus:outline-none resize-none"
 />
 ...
 className={`bg-foreground/[0.05] ${module.color} hover:opacity-90 disabled:opacity-50`}
 >
 Submit Reflection
 <ChevronRight className="w-4 h-4 ml-2" />
 </Button>
 ...
 <p className="text-sm ink-vermilion mt-2 pl-7">
 {q.explanation}
 </p>
 )}
 </div>
 ...
 className={`mt-6 bg-foreground/[0.05] ${module.color} hover:opacity-90 disabled:opacity-50`}
 >
 Submit Quiz
 <ChevronRight className="w-4 h-4 ml-2" />
 </Button>
 ) : (
 <div className="mt-6 p-4 rounded-xl bg-foreground/30 border border-foreground/30">
 ...
 <div className={`w-24 h-24 rounded-full bg-foreground/[0.05] ${module.color} flex items-center justify-center mx-auto mb-6`}>
 ...
 <p className="text-2xl font-bold ink-vermilion">{totalScore}</p>
 <p className="text-sm text-gray-400">Points Earned</p>
 </div>
 <div className="p-4 rounded-xl bg-black border border-white/30">
 <p className="text-2xl font-bold text-foreground">{completedScenes}</p>
 ...
 <span
 key={i}
 className={`px-3 py-1 rounded-full text-sm bg-foreground/[0.05] ${module.color} bg-opacity-20 text-white`}
 >
 {comp}
 </span>
 ))}
 ...
 className={`bg-foreground/[0.05] ${module.color} hover:opacity-90`}
 >
 ...
 <CheckCircle className="w-8 h-8 ink-vermilion mx-auto mb-2" />
 <p className="ink-vermilion font-medium">Module Completed!</p>
 </div>
 ...
 {/* Footer navigation */}
 <div className="absolute bottom-0 left-0 right-0 z-50 bg-foreground/80 backdrop-blur-xl border-t border-white/5">
 ...
 isCurrent
 ? 'w-6 h-2 bg-foreground/10'
 : isCompleted
 ? 'w-2 h-2 bg-vermilion/10'
 : 'w-2 h-2 bg-foreground/[0.05]'
 }`}
 />
 );
 })}
 ...
 className={`bg-foreground/[0.05] ${module.color} hover:opacity-90 disabled:opacity-30 px-6 shadow-lg`}
 >
 Continue
 <ChevronRight className="w-4 h-4 ml-2" />
 </Button>
