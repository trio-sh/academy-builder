...
export const InteractiveSkillAssessment = () => {
 ...
 <AlertCircle className="w-12 h-12 ink-vermilion mx-auto mb-2" />
 <p className="ink-vermilion">Microphone access required for voice challenges.</p>
 <Button
 onClick={async () => {
 const permission = await requestMicrophonePermission();
 setMicPermission(permission);
 ...
 size="lg"
 className="bg-foreground/10 hover:bg-foreground/10 rounded-full w-16 h-16"
 >
 ...
 size="lg"
 className="bg-foreground/10 hover:bg-foreground/10 rounded-full w-16 h-16 animate-pulse"
 >
 ...
 {isRecording && (
 <div className="text-center">
 <p className="ink-vermilion font-mono text-xl">{formatTime(recordingTime)}</p>
 ...
 <CheckCircle className="w-6 h-6 ink-vermilion shrink-0 mt-0.5" />
 <div>
 <p className="ink-vermilion font-medium">Response recorded.</p>
 <p className="ink-vermilion text-sm mt-1">Scenario complete — your mentor will review.</p>
 </div>
 </div>
 </motion.div>
 )}
 ...
 <div className="px-3 py-1 rounded-full bg-foreground/20 text-foreground">
 Type: {challenge.type}
 </div>
 ...
 ? 'bg-foreground/20 ink-vermilion'
 : 'bg-foreground/20 text-gray-400'
 }`}>
 ...
 timeRemaining < 30 ? 'bg-foreground/20 ink-vermilion' : 'bg-vermilion/10 ink-vermilion'
 } w-fit`}>
 ...
 const urgencyColors = {
 critical: 'border-foreground/50 bg-foreground/30',
 high: 'border-vermilion bg-vermilion/10',
 medium: 'border-vermilion bg-vermilion/10',
 low: 'border-gray-500/50 bg-foreground/30'
 };
 ...
 <div className="space-y-6">
 <div className="p-4 rounded-xl bg-foreground/30 border border-foreground/20">
 <p className="text-foreground font-medium mb-2">{rolePlay.title}</p>
 ...
 <div className="max-w-[80%] p-4 rounded-2xl bg-foreground/[0.05] text-gray-300">
 <p className="text-xs text-foreground mb-1">{rolePlay.aiRole}</p>
 <p>{turn.message}</p>
 </div>
 )}
 ...
 className="w-full p-3 text-left rounded-xl bg-black border border-white/30 hover:bg-black hover:border-foreground/50 transition-all"
 >
 ...
 initial={{ opacity: 0 }}
 animate={{ opacity: 1 }}
 className="p-3 rounded-lg bg-black border-l-4 border-foreground/25"
 >
 ...
 <div className="space-y-4">
 <p className="ink-vermilion text-sm">Audio completed. Answer the questions below:</p>
 ...
 >
 <p className="ink-vermilion text-sm">Response recorded.</p>
 </motion.div>
 )}
 </button>
 ))}
 ...
 <div className={`flex items-center gap-2 px-3 py-2 rounded-full ${
 timeRemaining < 10 ? 'bg-foreground/20 ink-vermilion animate-pulse' : 'bg-vermilion/10 ink-vermilion'
 } w-fit`}>
 ...
 <div className="w-24 h-24 rounded-full bg-foreground/[0.05] flex items-center justify-center mx-auto mb-6">
 <CheckCircle className="w-12 h-12 text-white" />
 </div>

 <h2 className="text-2xl font-bold text-white mb-3">Session Recorded.</h2>
 <p className="text-gray-400 max-w-md mx-auto mb-2">
 Scenario complete — your mentor will review.
 </p>
 <p className="text-gray-500 text-sm max-w-md mx-auto mb-8">
 Your behavioral evidence has been logged. No re-attempts are available — your mentor will be in touch with next steps.
 </p>
 ...
 case 'voice-response':
 return <Mic className="w-5 h-5 ink-vermilion" />;
 case 'written-challenge':
 ...
 case 'judgment':
 return <Briefcase className="w-5 h-5 text-foreground" />;
 case 'quick-response':
 ...
 // ── Build Rule 9 — Cooldown blocking screens ─────────────────────
 // Loading: checking cooldown status
 ...
 <p className="text-gray-400">Checking session eligibility...</p>
 </div>
 </div>
 );
 }
 ...
 if (cooldownStatus && !cooldownStatus.allowed) {
 const endsAt = cooldownStatus.cooldownEndsAt;
 ...
 {cooldownStatus.daysRemaining && cooldownStatus.daysRemaining > 1 && (
 <span className="text-gray-500"> ({cooldownStatus.daysRemaining} days)</span>
 )}
 </p>
 <div className="p-4 rounded-xl bg-foreground/30 border border-foreground/20 mb-6 text-left">
 ...
 animate={{ opacity: 1, y: 0 }}
 exit={{ opacity: 0, y: -20 }}
 className="bg-foreground/[0.05] rounded-3xl border border-white/30 overflow-hidden backdrop-blur-sm shadow-2xl"
 >
 {/* Scene header */}
