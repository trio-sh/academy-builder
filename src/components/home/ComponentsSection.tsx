...
function ComponentCard({ component, index }: ComponentCardProps) {
  return (
    ...
    >
      ...
      {/* Glass card */}
      <div className="relative h-full bg-black border border-white/30 rounded-3xl p-8 group-hover:border-white/30 transition-all duration-500 overflow-hidden">
        {/* Icon */}
        <div
          className={cn(
        ...
        <div className="flex flex-wrap gap-2">
          {component.features.slice(0, 2).map((feature) => (
            <span
              key={feature}
              className="px-3 py-1 text-xs rounded-full bg-black text-gray-50 group-hover:bg-white/15 group-hover:text-white transition-colors"
            >
              {feature}
            </span>
          ))}
